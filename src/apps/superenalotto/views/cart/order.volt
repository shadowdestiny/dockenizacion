{% extends "../../shared/views/cart/order.volt" %}
...
{% block template_scripts_code %}
    ...
    $(document).on("moneymatrix",{wallet: true},function(e, wallet) {
        ...
        disableiframeclick = true;
         ...
         $.post('/cart/iframereload', "tsid="+tsid+"&wallet="+wallet+"&lottery=SuperEnalotto",function(response){
...
{% block body %}
  ...
  {% set cashierURL=cashier.cashierUrl %}
        ...
        <div class="wrapper">
            <div class="review_and_pay-section">
                ...
                <div class="payment hidden">
                    {% if cashier.message != "" %}
                      {% include "../../shared/views/cart/loading_order_processing.volt" %}
                    {% elseif cashier.cashierUrl != null %}
                      {% include "../../shared/views/cart/moneymatrix_iframe.volt" %}
                    {% else %}
                    <section class="section--card--details">
                        ...
                        <div class="section--content">
                            <form class="box-add-card form-currency {#{% if which_form != 'edit' and which_form%}hidden{% endif %}#}"
                                  method="post"
                                  action="/superenalotto/payment{#{% if which_form == 'edit'%}/account/editPayment/{{ payment_method.id_payment }}{% else %}/{% endif %}#}">
    ...
    {% include "../../shared/views/_elements/footer.volt" %}
{% endblock %}