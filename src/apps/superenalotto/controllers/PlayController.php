...
use Money\Currency;
use EuroMillions\web\vo\cutoff\SuperEnalottoCutOff;
...
final class PlayController extends \EuroMillions\shared\controllers\PlayController
{
    ...
    public function indexAction()
    {
        ...
        $this->singleBetPrice = $this->lotteryService->getSingleBetPriceByLottery('SuperEnalotto');
        ...
        $this->play_dates = $this->lotteryService->getRecurrentDrawDates('SuperEnalotto');
        $this->draw = $this->lotteryService->getNextDateDrawByLottery('SuperEnalotto');
        $dateTimeToClose =  $this->lotteryService->getNextDateDrawByLottery('SuperEnalotto',null,true);
        $date_time_util = new DateTimeUtil();
        $this->dayOfWeek = $date_time_util->getDayOfWeek($this->draw);
        $this->checkOpenTicket = (new SuperEnalottoCutOff($dateTimeToClose))->isClosed();
        $this->singleBetPrice = $this->lotteryService->getSingleBetPriceByLottery('SuperEnalotto');
        $this->automaticRandom = $this->request->get('random');
        $this->bundlePriceDTO = $this->domainServiceFactory->getPlayService()->retrieveEuromillionsBundlePriceDTO('SuperEnalotto');
        $single_bet_price = $this->lotteryService->getSingleBetPriceByLottery('SuperEnalotto');
        ...
        return $this->view->setVars([
            ...
            'pageController' => 'superenalottoPlay',
