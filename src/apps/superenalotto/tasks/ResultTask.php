<?php
namespace EuroMillions\superenalotto\tasks;
...
class ResultTask extends TaskBase
{
    ...
    public function importAllHistoricalDataFromSuperEnalottoAction()
    {
        try {
            $dependencies = [];
            ...
            $results = $this->lotteryService->getAllResultFromLottery(new Curl(), Di::getDefault()->get('config')['superenalotto_api'], 'superenalotto');
            $this->lotteriesDataService->insertLotteryData($results->body,$dependencies, 'SuperEnalotto');
        } catch (Exception $e) {
            throw new \Exception($e->getMessage());
        }
    }

    public function updateSuperEnalottoResultAction(\DateTime $now = null)
    {
        try {
            $resultConfigQueue = $this->di->get('config')['aws']['queue_results_endpoint'];
            $drawDate = $this->lotteryService->getLastDrawDate('SuperEnalotto');
            $result = $this->lotteryService->getLastResult('SuperEnalotto');
            $breakdown = $this->lotteryService->getLastBreakdown('SuperEnalotto');
            if (!$result['regular_numbers'][0])
            {
                $this->lotteriesDataService->updateLastDrawResultLottery('SuperEnalotto');
            }
            if(!empty($breakdown) && !$breakdown->getCategoryOne()->getName())
            {
                $this->lotteriesDataService->updateLastBreakDownLottery('SuperEnalotto');
            }
        }catch (\Exception $e)
        {
