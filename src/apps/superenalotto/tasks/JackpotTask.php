...
class JackpotTask extends TaskBase
{
    ...
    public function updateNextDrawJackpotSuperEnalottoAction()
    {
        $this->lotteriesDataService->updateNextDrawJackpotLottery('SuperEnalotto');
    ...
    public function updatePreviousAction(\DateTime $today = null)
    {
        ...
        /** @var \DateTime $date */
        $date = $this->lotteryService->getLastDrawDate('SuperEnalotto', $today);
        $this->lotteriesDataService->updateNextDrawJackpotLottery('SuperEnalotto', $date->sub(new \DateInterval('PT1M')));
    }

    public function reminderJackpotAction()
    {
        $jackpot_amount = $this->lotteryService->getNextJackpot('SuperEnalotto');
