    ...
    <div class="wrapper">
        <div class="box-basic">
            ...
            {% for currency in currency_list %}
                <li><a data-enhance=false href="/ajax/user-settings/setCurrencyReload/{{currency.code}}" class="{% if currency.code == current_currency %} active {% endif %}"><span class="curr">{{ currency.code }} {{ currency.symbol }}</span> <span class="name">{{ currency.name }}</span></a></li>
            {% endfor %}
            </ul>
        </div>
    </div>
