    ...
    public function getDepositorsD1($data)
    {
        ...
        return $this->entityManager
            ->createNativeQuery("
                ...
                FROM transactions t 
                LEFT JOIN users u ON u.id = t.user_id 
                WHERE date BETWEEN DATE_FORMAT(DATE_ADD(created, INTERVAL +2 DAY), '%Y-%m-%d 00:00:01') AND DATE_FORMAT(DATE_ADD(created, INTERVAL +3 DAY), '%Y-%m-%d 00:00:01') 
                AND created BETWEEN '" . date('Y-m-d', strtotime($data['dateFrom'])) . " 00:00:01' AND '" . date('Y-m-d', strtotime($data['dateTo'])) . " 23:59:59'
                AND u.country IN ('" . implode("','", $data['countries']) . "')
                AND t.entity_type IN ('deposit', 'subscription_purchase') GROUP BY u.id", $rsm)->getResult();
    }
    ...
    public function getDepositorsD2($data)
    {
        ...
        return $this->entityManager
            ->createNativeQuery("
                ...
                FROM transactions t 
                LEFT JOIN users u ON u.id = t.user_id 
                WHERE date BETWEEN DATE(DATE_ADD(created, INTERVAL +3 DAY)) AND '" . date('Y-m-d', strtotime($data['dateTo'])) . " 23:59:59'
                AND created BETWEEN '" . date('Y-m-d', strtotime($data['dateFrom'])) . " 00:00:01' AND '" . date('Y-m-d', strtotime($data['dateTo'])) . " 23:59:59'
                AND u.country IN ('" . implode("','", $data['countries']) . "')
                AND t.entity_type IN ('deposit', 'subscription_purchase') GROUP BY u.id", $rsm)->getResult();
    }
