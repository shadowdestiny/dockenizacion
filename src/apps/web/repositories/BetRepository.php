...
//        $rsm->addFieldResult('u','userId','id');
    ...
    public function getMatchesPlayConfigAndUserFromLotteryByDrawDate($date, $lotteryId)
    {
        $rsm = new ResultSetMapping;
        //$rsm->addEntityResult('EuroMillions\web\entities\User', 'u');
        // $rsm->addJoinedEntityResult('EuroMillions\web\entities\PlayConfig','p','b','playConfig');
...
//        $rsm->addFieldResult('u','userId','id');
        ...
        $result = $this->getEntityManager()
            ->createNativeQuery("
                ...
                from bets b
                  ...
                  INNER JOIN euromillions_draws e on e.id=b.euromillions_draw_id
                  INNER JOIN users u ON u.id = p.user_id
                WHERE p.active=1 and e.draw_date = ? and p.lottery_id=".$lotteryId." 
    ...
    public function getMatchesPlayConfigAndUserFromEuroJackpotByDrawDate($date, $lotteryId)
    {
        $rsm = new ResultSetMapping;
        $rsm->addScalarResult('userId','userId');
        $rsm->addScalarResult('bet','bet');
    ...
    public function getMatchesPlayConfigAndUserFromMegaSenaByDrawDate($date, $lotteryId)
    {
        $rsm = new ResultSetMapping;
        $rsm->addScalarResult('userId','userId');
        $rsm->addScalarResult('bet','bet');
            ...
            ->createNativeQuery("
                SELECT p.id, u.id as userId, b.playConfig_id as playConfig, b.id as bet, b.euromillions_draw_id as draw,
                             ...
                             IF(p.line_regular_number_five IN (e.result_regular_number_one,
                              ...
                              (IF(p.line_lucky_number_two IN (e.result_regular_number_one,
                                                              e.result_regular_number_two,
                                                              e.result_regular_number_three,
                                                              e.result_regular_number_four,
                                                              e.result_regular_number_five,
                ...
                from bets b
                  ...
                  INNER JOIN euromillions_draws e on e.id=b.euromillions_draw_id
                  INNER JOIN users u ON u.id = p.user_id
                WHERE p.active=1 and e.draw_date = ? and p.lottery_id= ?
                having (cnt=5 and cnt_lucky=1) OR (cnt=5 and cnt_lucky=0) OR (cnt=4 and cnt_lucky=1)
                    OR (cnt=4 and cnt_lucky=0) OR (cnt=3 and cnt_lucky=1)
            ",$rsm);
        $result->setParameter(1, $date);
        $result->setParameter(2, $lotteryId);
        return $result->getResult();

    }

    public function getMatchesPlayConfigAndUserFromSuperEnalottoByDrawDate($date, $lotteryId)
    {
        $rsm = new ResultSetMapping;
        $rsm->addScalarResult('userId','userId');
        $rsm->addScalarResult('bet','bet');
            ...
            ->createNativeQuery("
                SELECT p.id, u.id as userId, b.playConfig_id as playConfig, b.id as bet, b.euromillions_draw_id as draw,
                             ...
                             IF(p.line_lucky_number_two IN (e.result_regular_number_one,
                                                              ...
                                                              e.result_regular_number_four,
                                                              e.result_regular_number_five,
                                                              e.result_lucky_number_two), 1, 0)) as cnt,
                             (IF(e.result_lucky_number_one IN (p.line_regular_number_one,
                                                              p.line_regular_number_two,
                                                              p.line_regular_number_three,
                                                              p.line_regular_number_four,
                                                              p.line_regular_number_five,
                                                              p.line_lucky_number_two), 1, 0)) as cnt_lucky
                from bets b
                  INNER JOIN play_configs p ON b.playConfig_id=p.id
                  INNER JOIN euromillions_draws e on e.id=b.euromillions_draw_id
                  INNER JOIN users u ON u.id = p.user_id
                WHERE p.active=1 and e.draw_date = ? and p.lottery_id= ?
                having cnt=6 OR cnt=5 OR cnt=4 OR cnt=3
            ",$rsm);
        $result->setParameter(1, $date);
        $result->setParameter(2, $lotteryId);
        return $result->getResult();
    ...
    public function getPastGamesWithPrizes($userId)
    {
        $result = $this->getEntityManager()
            ->createQuery(
                ...
                . ' ORDER BY e.id ASC, p.startDrawDate ASC ')
