...
class WebLanguageStrategy implements ILanguageStrategy
{
    ...
    public function get()
    {
        if (!empty(Di::getDefault()->get('router')->getParams()['language'])) {
            $this->session->set(self::LANGUAGE_VAR, Di::getDefault()->get('router')->getParams()['language']);
            $language = Di::getDefault()->get('router')->getParams()['language'];
        } else {
