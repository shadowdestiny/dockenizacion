...
<table class="table-program" width="100%" id="tableExport">
    ...
    {% for date in arrayDates %}
        ...
        <td>{% if arrayTotals[date]['newDepositors'] is defined and arrayTotals[date]['newRegistrations'] is defined %}{{ arrayTotals[date]['newDepositors'] / (arrayTotals[date]['newRegistrations'] * 100) }}{% else %}0{% endif %}</td>
        ...
        <td>{% if arrayTotals[date]['depositAmount'] is defined %}{{ arrayTotals[date]['depositAmount'] }}{% else %}0{% endif %}</td>
        <td>{% if arrayTotals[date]['numberWithdrawals'] is defined %}{{ arrayTotals[date]['numberWithdrawals'] }}{% else %}0{% endif %}</td>
        <td>{% if arrayTotals[date]['withdrawalAmount'] is defined %}{{ arrayTotals[date]['withdrawalAmount'] }}{% else %}0{% endif %}</td>
        <td>{% if arrayTotals[date]['playerWinnings'] is defined %}{{ arrayTotals[date]['playerWinnings'] }}{% else %}0{% endif %}</td>
