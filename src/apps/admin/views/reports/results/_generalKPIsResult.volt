...
<table class="table-program" width="100%" id="tableExport">
    ...
    {% for date in arrayDates %}
        ...
        <td>{% if arrayTotals[date]['grossGaming'] is defined and arrayTotals[date]['actives'] is defined %}{{ (((kpi['grossGaming']/100) | number_format (2,',','')) / kpi['actives']) | number_format (2,',','')  }}{% else %}0{% endif %}</td>
        {% for key, kpi in generalKPIs[date] %}
            <tr>
                ...
                <td class="values">{% if kpi['grossGaming'] is defined and kpi['actives'] is defined%}{{ (((kpi['grossGaming']/100) | number_format (2,',','')) / kpi['actives']) | number_format (2,',','') }}{% else %}0{% endif %}</td>
            </tr>
        {% endfor %}
        </tr>
    {% endfor %}
    <tr>
        ...
        <td>{% if total['grossGaming'] is defined and total['actives'] is defined%}{{ (((kpi['grossGaming']/100) | number_format (2,',','')) / kpi['actives']) | number_format (2,',','') }}{% else %}0{% endif %}</td>
