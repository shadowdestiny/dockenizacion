...
class TestListener extends \PHPUnit_Framework_BaseTestListener
{
    public function startTestSuite(PHPUnit_Framework_TestSuite $suite)
    {
        if ($suite->getName() == "integration") {
            ...
            exec($command);
            $bootstrap = new Bootstrap(new TestWebBootstrapStrategy(false, APP_PATH,APP_PATH . 'config/'  , APP_PATH . 'assets/', TESTS_PATH . '/../', 'testconfig.ini'));
        } else {
            $bootstrap = new Bootstrap(new TestWebBootstrapStrategy(true, APP_PATH, APP_PATH . 'config/' , APP_PATH . 'assets/', TESTS_PATH . '/../', 'testconfig.ini'));
        }
        $bootstrap->execute();
    }
}